...
#include "srsenb/hdr/stack/mac/schedulers/sched_time_cr.h"
#include <vector>
...
sched_time_cr::sched_time_cr(const sched_cell_params_t& cell_params_, const sched_interface::sched_args_t& sched_args)
{
  cc_cfg = &cell_params_;
  ...
  dl_queue = ue_dl_queue_t(ue_dl_prio_compare{}, std::move(dl_storage)); //add to queue
...
void sched_time_cr::new_tti(sched_ue_list& ue_db, sf_sched* tti_sched)
{
  ...
  for (auto& u : ue_db) {
    ...
    if (it == ue_history_db.end()) {
      it = ue_history_db.insert(u.first, ue_ctxt{u.first, u.second.get()->slice, fairness_coeff}).value();
    }
...
void sched_time_cr::sched_dl_users(sched_ue_list& ue_db, sf_sched* tti_sched)
{
...
uint32_t sched_time_cr::try_dl_alloc(ue_ctxt& ue_ctxt, sched_ue& ue, sf_sched* tti_sched)
{
  ...
  if (code != alloc_result::no_cch_space and ue_ctxt.dl_newtx_h != nullptr) {
    ...
    if (cc_occupied == true){
      code = try_dl_newtx_alloc_cr(*tti_sched, ue, *ue_ctxt.dl_newtx_h, &alloc_mask);
    }
    else{
...
void sched_time_cr::sched_ul_users(sched_ue_list& ue_db, sf_sched* tti_sched)
{
  srsran::tti_point tti_rx{tti_sched->get_tti_rx()};
  if (current_tti_rx != tti_rx) {
    new_tti(ue_db, tti_sched);
...
uint32_t sched_time_cr::try_ul_alloc(ue_ctxt& ue_ctxt, sched_ue& ue, sf_sched* tti_sched)
{
...
void sched_time_cr::ue_ctxt::new_tti(const sched_cell_params_t& cell, sched_ue& ue, sf_sched* tti_sched)
{
  dl_retx_h  = nullptr;
  dl_newtx_h = nullptr;
  ul_h       = nullptr;
  ...
  if (dl_retx_h != nullptr or dl_newtx_h != nullptr) {
    ...
    // priority of slice 1 > priority of slice 2 > priority of slice 3
...
void sched_time_cr::ue_ctxt::save_dl_alloc(uint32_t alloc_bytes, float exp_avg_alpha)
{
...
void sched_time_cr::ue_ctxt::save_ul_alloc(uint32_t alloc_bytes, float exp_avg_alpha)
{
...
/* Compare the priority of two UEs
 * 1. The UEs from slice 1 > The UEs from slice 2 > The UEs from slice 3
 * 2, Within the Slice, Proportional fair
 */

/*
 * Dl queue priority, compare slice first, then compare if this is a retx, last compare the pf priority.
 */
bool sched_time_cr::ue_dl_prio_compare::operator()(const sched_time_cr::ue_ctxt* lhs,
                                                   const sched_time_cr::ue_ctxt* rhs) const
{
  ...
  else if(rhs->slice<lhs->slice){
    ret = true;
  }
  else{
    ret = (not is_retx1 and is_retx2) or (is_retx1 == is_retx2 and lhs->dl_prio < rhs->dl_prio);
  }
  return ret;
}

bool sched_time_cr::ue_ul_prio_compare::operator()(const sched_time_cr::ue_ctxt* lhs,
                                                   const sched_time_cr::ue_ctxt* rhs) const
{
  ...
  if (lhs->slice<rhs->slice){
    ...
    ret = (not is_retx1 and is_retx2) or (is_retx1 == is_retx2 and lhs->ul_prio < rhs->ul_prio);
  }
  return ret;
}

void sched_time_cr::set_reserved_carrier(uint32_t enb_cc_idx){
  reserved_carrier = enb_cc_idx;
}

// Record occupation of the reserved carrier in a circular buffer
void sched_time_cr::record_cc_result(rbgmask_t dl_mask){

  float reservation_lv = 0; // garenteed bandwidth for slice 1, from 0-1
  if (dl_mask.size() != 0 and dl_mask.nof_words_()/dl_mask.size() < reservation_lv){
    reserved_cc_occupied_result.insert(current_tti_rx.to_uint(), (dl_mask.nof_words_()/dl_mask.size()));
  } 
  else if (dl_mask.size() != 0 and dl_mask.nof_words_()/dl_mask.size() >= reservation_lv){
    for (auto it = reserved_cc_occupied_result.begin(); it != reserved_cc_occupied_result.end();){
      it = reserved_cc_occupied_result.erase(it);
    }
    cc_occupied = true;
  }

  if (reserved_cc_occupied_result.full()){
    printf("cc_not_occupied");
    cc_occupied = false;
  }
