...
void sched_ue::set_cfg(const ue_cfg_t& cfg_)
{
  ...
  if (prev_supported_cc_list.empty() or is_handover) {
    logger.info("SCHED: rnti=0x%x PCell is now Carrier Component id = %d", rnti, cfg.supported_cc_list[0].enb_cc_idx);
  }
...
// assign user to a slice
void sched_ue::set_slice(uint8_t slice_){
  
  if (slice_ == 1 or slice_ == 2 or slice_ == 3){slice = slice_;}

}

uint8_t sched_ue::get_slice(){
  return slice;
}
