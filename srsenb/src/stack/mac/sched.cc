...
///Xiaoming Song
void  sched::slice_sched_cfg(uint16_t rnti, uint8_t slice){
        ue_db[rnti]->set_slice(slice); // function to set QCI in sched_ue
...
void sched::new_tti(tti_point tti_rx)
{
  ...
  for (size_t cc_idx = 0; cc_idx < carrier_schedulers.size(); ++cc_idx) {
    if (not is_generated(tti_rx, cc_idx)) {
      ...
      carrier_schedulers[cc_idx]->generate_tti_result(tti_rx); ///Xiaoming
    }
  }
}
