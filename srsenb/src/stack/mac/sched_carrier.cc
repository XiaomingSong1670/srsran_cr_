...
void sched::carrier_sched::carrier_cfg(const sched_cell_params_t& cell_params_)
{
  ...
  if (cell_params_.sched_cfg->sched_policy == "time_pf") {
  ...
  else if (cell_params_.sched_cfg->sched_policy == "time_cr"){
    sched_algo.reset(new sched_time_cr{*cc_cfg, *cell_params_.sched_cfg});
    logger.info("Using time-domain CR scheduling policy for cc=%d", cc_cfg->enb_cc_idx);
  }
  else
  {
...
const cc_sched_result& sched::carrier_sched::generate_tti_result(tti_point tti_rx)
{
  ...
  if (cc_cfg->enb_cc_idx == 0){
    sched_algo->record_cc_result(cc_result->dl_mask);
  }
